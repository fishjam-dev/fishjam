defmodule JellyfishWeb.Integration.PeerSocketTest do
  ...
  test "peer removal", %{room_id: room_id, peer_id: peer_id, token: token, conn: conn} do
    ...
    assert_receive {:disconnected, {:remote, 1000, "peer_removed"}}, 1000
  end
  ...
  test "room crash", %{room_pid: room_pid, token: token} do
    ...
    assert_receive {:disconnected, {:remote, 1011, "internal_server_error"}}, 1000
    assert_receive {:DOWN, ^ref, :process, ^ws, {:remote, 1011, "internal_server_error"}}
  end
  ...
  test "room close", %{room_id: room_id, token: token, conn: conn} do
    ...
    assert_receive {:disconnected, {:remote, 1000, "room_stopped"}}, 1000
  end
