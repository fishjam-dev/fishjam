...
defmodule Example do
  ...
  @jellyfish_port 4000
  @stream_uri "rtsp://wowzaec2demo.streamlock.net:554/vod/mp4:BigBuckBunny_115k.mp4"
  ...
  def run() do
    ...
    with {:ok, %Jellyfish.Room{id: room_id}} <- Jellyfish.Room.create(client),
         {:ok, %Jellyfish.Component{id: _hls_component_id}} <- Jellyfish.Room.add_component(client, room_id, "hls"),
         {:ok, %Jellyfish.Component{id: _rtsp_component_id}} <- Jellyfish.Room.add_component(client, room_id, "rtsp", source_uri: @stream_uri) do
